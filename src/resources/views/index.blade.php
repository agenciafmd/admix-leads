...
@section('batch')
    ...
    @else
        @can('delete', '\Agenciafmd\Leads\Lead')
            {{ Form::select('batch', ['' => 'com os selecionados', route('admix.leads.batchDestroy') => '- remover', route('admix.leads.batchExport') => '- exportar', route('admix.leads.batchExport', ['all' => true] + ['filter' => (request()->filter ?? [])]) => '- exportar todos (' . $items->total() . ' itens)',], null, ['class' => 'js-batch-select form-control custom-select']) }}
        @endcan
    @endif
@endsection
